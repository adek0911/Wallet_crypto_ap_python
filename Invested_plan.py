...
def invested_area_ingredients(
    ...
    def count_predict():
        ...
        if value[0] == "":
            value[0] = convert_price(value[0], value[1], dollar_price)["PLN"]
            invested_area.objList[1].insert(0, str(value[0]))

        if value[1] == "":
            value[1] = convert_price(value[0], value[1], dollar_price)["USD"]
            invested_area.objList[3].insert(0, str(value[1]))
        ...
        children_index = area_dict["middle_area"].objList[1].get_children()[combo_index]

        quantity = (
            float(value[0])
            / float(
                area_dict["middle_area"].objList[1].item(children_index)["values"][0]
            )
        ).__round__(4)
