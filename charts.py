...
def load_from_api(currency: str, url: str) -> list:
    url = requests.get(f"{url}chart_currency/{currency}")
...
def chart_area_result(area_frame_obj, url: str):
    ...
    def selected_combobox(event):
        ...
        file_data = load_from_api(choice, url)
        ...
        for i in range(3):
            ...
            wykres = plt.Figure(
                dpi=96, facecolor=("#375a7f")
            )  # DPI 96 Large monitor 120 in small monitor
            # For pc resolution
            wykres.set_size_inches(4.55, 1.6)  # 4.2 , 1.6
            # for laptop resolution
            # wykres.set_size_inches(3.4, 0.65)  # 0.5
